 ...
 * @license    MIT License
 */
namespace Propel\Bundle\PropelAclBundle\Command;

use Propel\PropelBundle\Command\SqlInsertCommand;
...
EOT
    ...
    {
        $aclSchema = new \SplFileInfo($kernel->locateResource('@PropelBundle/Resources/config/propel/acl_schema.xml'));

        return array(
            (string) $aclSchema => array($kernel->getBundle('PropelBundle'), $aclSchema),
        );
    }
    ...
    protected function getSqlDir()
    {
        return implode(DIRECTORY_SEPARATOR, array(
            $this->getApplication()->getKernel()->getCacheDir(),
            'propel', 'acl', 'sql',
        ));
    }
}
